const state = {
    ...
    // ⚡ Performance: Caches for high-frequency updates
    domCache: {},
    lastValues: {},
...
function renderResults() {
    ...
    // ⚡ Performance: Reset caches on re-render to ensure fresh references and values
    state.domCache = {};
    state.lastValues = {};
...
// ⚡ Performance: Accept Date object to avoid redundant parsing
...
// ⚡ Performance: Optimized update helper with dirty checking and DOM caching
...
function startLiveUpdates() {
    // ⚡ Performance: Clear existing interval if any to prevent leaks
    if (state.liveUpdateInterval) clearInterval(state.liveUpdateInterval);

    state.liveUpdateInterval = setInterval(() => {
        if (!state.user.dob || !state.user.dobDate) return;

        // ⚡ Performance: Use cached Date object
        const diff = new Date() - state.user.dobDate;

        updateStat('val-seconds', state.formatter.format(Math.floor(diff / 1000)));
        updateStat('val-years', calculateAge(state.user.dobDate).years);
        updateStat('val-months', state.formatter.format(Math.floor(diff / 2629800000)));
        updateStat('val-weeks', state.formatter.format(Math.floor(diff / 604800000)));
        updateStat('val-days', state.formatter.format(Math.floor(diff / 86400000)));
        updateStat('val-hours', state.formatter.format(Math.floor(diff / 3600000)));
        updateStat('val-minutes', state.formatter.format(Math.floor(diff / 60000)));

        // ⚡ Performance: Run expensive date formatting only once
        ...
        updateStat('est-sleep', state.formatter.format(Math.floor(days * 8)));
        updateStat('est-eat', state.formatter.format(Math.floor(days * 1.5)));
