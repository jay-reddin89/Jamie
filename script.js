...
/**
 * Calculates age components from DOB.
 * Bolt Optimization: Accepts Date objects to avoid redundant parsing.
 */
 ...
 * Bolt Optimization: DOM caching, dirty checking, pre-instantiated formatter, and minimized object creation.
 */
function startLiveUpdates() {
    ...
    if (bornDayEl) bornDayEl.textContent = dob.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
    ...
    state.liveUpdateInterval = setInterval(() => {
        ...
        const update = (id, val) => {
            ...
            if (!state.domCache[id]) state.domCache[id] = document.getElementById(id);
        ...
        // Numerical updates with pre-instantiated formatter
        update('val-seconds', state.formatter.format(Math.floor(diff / 1000)));
        update('val-years', age.years);
        update('val-months', state.formatter.format(Math.floor(diff / 2629800000)));
        update('val-weeks', state.formatter.format(Math.floor(diff / 604800000)));
        update('val-days', state.formatter.format(Math.floor(diff / 86400000)));
        update('val-hours', state.formatter.format(Math.floor(diff / 3600000)));
        update('val-minutes', state.formatter.format(age.minutes));
 ...
 * Bolt Optimization: Uses pre-instantiated formatter for fallback.
 */
