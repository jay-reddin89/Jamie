...
// Hoisted for performance: Reuse a single formatter instead of creating one per tick
...
/**
 * High-frequency update loop (1s interval) optimized for zero-allocation and minimal DOM overhead.
 */
function startLiveUpdates() {
    if (!state.user.dob) return;

    // Cache element references to eliminate O(N) getElementById calls per tick
    const elementsCache = {
    ...
    // Pre-calculate constants and static strings to reduce per-tick workload
    const birthDateObj = new Date(state.user.dob);
    const birthTimestamp = birthDateObj.getTime();
    const bornDayString = birthDateObj.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();

    // Reusable update helper using the cache
    const update = (key, val) => {
        const el = elementsCache[key];
    ...
    state.liveUpdateInterval = setInterval(() => {
        ...
        // Fast calculations using pre-cached timestamp
        ...
        // UI Updates using hoisted liveFormatter for ~5-10x faster formatting vs toLocaleString()
        update('seconds', liveFormatter.format(seconds));
        update('years', years);
        update('months', liveFormatter.format(Math.floor(diff / 2629800000)));
        update('weeks', liveFormatter.format(Math.floor(diff / 604800000)));
        update('days', liveFormatter.format(Math.floor(days)));
        update('hours', liveFormatter.format(hours));
        update('minutes', liveFormatter.format(minutes));
        update('bornDay', bornDayString);

        update('heart', formatLarge(minutes * 72));
        update('breaths', formatLarge(minutes * 14));
        update('sleep', formatLarge(days * 8));
        update('eat', formatLarge(days * 1.5));
        update('blinks', formatLarge(days * 15 * 60 * 16));
    }, 1000);
}
