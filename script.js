const state = {
    ...
    settings: {
        sections: ['realtime', 'facts', 'livedthrough', 'top', 'standing', 'astronomical', 'transit', 'economic', 'tech', 'network', 'eco', 'power', 'knowledge', 'namemeaning', 'nameworld', 'namejokes', 'futuremilestones', 'positivelegacy', 'lifemap']
    },
...
function renderResults() {
    ...
    const orderedKeys = ['realtime', 'facts', 'livedthrough', 'top', 'standing', 'astronomical', 'transit', 'economic', 'tech', 'network', 'eco', 'power', 'knowledge', 'namemeaning', 'nameworld', 'namejokes', 'futuremilestones', 'positivelegacy', 'lifemap'];
    ...
    orderedKeys.forEach(key => {
        if (sections.includes(key)) {
            switch(key) {
                ...
                case 'futuremilestones': renderFutureMilestones(); break;
                case 'positivelegacy': renderPositiveLegacy(); break;
                case 'lifemap': renderLifeMap(); break;
            }
        }
    });
    ...
    footer.innerHTML = `
        ...
        <button class="reinit-btn" id="share-snapshot-btn" style="background: var(--md-sys-color-secondary) !important; color: #000 !important;"><span>📸</span> SHARE LEGACY SNAPSHOT</button>
    `;
    elements.resultsSection.appendChild(footer);

    document.getElementById('download-pdf-btn').addEventListener('click', generatePDF);
    document.getElementById('share-snapshot-btn').addEventListener('click', showSocialSnapshot);

    startLiveUpdates();
}

function showSocialSnapshot() {
    ...
    const content = `
        <div class="card snapshot-card">
            <div class="logo-v3 snapshot-logo">LIFE.STATS</div>
            <div class="profile-card" style="margin-bottom: 24px; border: none; box-shadow: none;">
                <div class="avatar-hex">${state.user.profilePic ? `<img src="${state.user.profilePic}" class="avatar-img">` : '🧬'}</div>
                <div>
                    <h2 style="margin: 0;">${state.user.name.toUpperCase()}</h2>
                    <div class="item-subtitle">SINCE ${new Date(state.user.dob).getFullYear()}</div>
                </div>
            </div>
            <div class="snapshot-stats-primary">
                <div class="item-subtitle" style="color: var(--md-sys-color-secondary);">TOTAL TIME ELAPSED</div>
                <div style="font-size: 2rem; font-weight: 800; color: #fff;">${age.years} YEARS</div>
                <div style="font-size: 0.8rem; color: #ccc;">${Math.floor((new Date() - new Date(state.user.dob)) / 1000).toLocaleString()} SECONDS</div>
            </div>
            <div class="snapshot-stats-grid">
                <div class="snapshot-stat-box">
                    <div class="item-subtitle">HEARTBEATS</div>
                    <div style="color: var(--accent-amber); font-weight: 700;">${formatLarge((age.minutes * 72))}</div>
                </div>
                <div class="snapshot-stat-box">
                    <div class="item-subtitle">SMILES</div>
                    <div style="color: var(--accent-green); font-weight: 700;">${formatLarge((age.years * 365 * 15))}</div>
                </div>
            </div>
            <div class="snapshot-footer">
                GENERATED BY LIFE.STATS // DATA SEQUENCE: 0XF93A
            </div>
...
function renderEventsLived() {
    ...
    const subDeep = createCollapsibleSubSection('DEEP DIVE: BIRTH YEAR');
    subDeep.content.id = 'timeline-deep';
    subDeep.content.className = 'data-list';
    ...
    mainCard.appendChild(subDeep.container);
...
function renderFutureMilestones() {
    const { container, content } = createCollapsibleSection('FUTURE MILESTONES');
    const dob = new Date(state.user.dob);
    const now = new Date();

    const milestones = [
        { label: '1 Billion Seconds', seconds: 1e9 },
        { label: '500k Minutes', seconds: 500000 * 60 },
        { label: '1 Million Minutes', seconds: 1e6 * 60 },
        { label: '10k Days', seconds: 10000 * 24 * 3600 },
        { label: '20k Days', seconds: 20000 * 24 * 3600 },
        { label: '30k Days', seconds: 30000 * 24 * 3600 },
        { label: '50 Years', seconds: 50 * 31557600 },
        { label: '75 Years', seconds: 75 * 31557600 },
        { label: '100 Years', seconds: 100 * 31557600 }
    ];

    const data = milestones.map(m => {
        const milestoneDate = new Date(dob.getTime() + m.seconds * 1000);
        const isPast = milestoneDate < now;
        return {
            label: m.label,
            value: milestoneDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
            subtitle: isPast ? 'COMPLETED' : 'UPCOMING'
        };
    });

    content.appendChild(createDataList(data));
    elements.resultsSection.appendChild(container);
}

function renderPositiveLegacy() {
    const { container, content } = createCollapsibleSection('POSITIVE LEGACY');
    const age = calculateAge(state.user.dob).years;
    const days = age * 365;

    content.appendChild(createDataList([
        { label: 'Estimated Smiles Shared', value: `~${formatLarge(days * 15)}` },
        { label: 'Meals Shared (EST)', value: `~${formatLarge(days * 1.5)}` },
        { label: 'Trees Worth of O2 Produced', value: `~${(age * 8).toFixed(0)} trees` },
        { label: 'Potential Lives Touched', value: `~${formatLarge(days * 2.5)}` },
        { label: 'Moments of Joy Created', value: `~${formatLarge(days * 10)}` }
    ]));
...
function renderLifeMap() {
    const { container, content } = createCollapsibleSection('LIFE MAP');
    const age = calculateAge(state.user.dob).years;
    const lifeExpectancy = 80;
    const progress = Math.min(100, (age / lifeExpectancy) * 100);

    const card = document.createElement('div');
    card.className = 'card';
    card.innerHTML = `
        <div class="item-title life-progress-title">LIFE PROGRESS (${age}/${lifeExpectancy} YEARS)</div>
        <div class="progress-bar life-progress-bar">
            <div class="progress-fill" style="width: ${progress}%; background: var(--accent-cyan);"></div>
        </div>
        <div class="flex-row justify-between item-subtitle">
            <span>BIRTH</span>
            <span>${progress.toFixed(1)}%</span>
            <span>DESTINY</span>
        </div>
        <hr class="divider">
        <div class="item-title life-progress-title" style="margin-top: 16px;">TIME ALLOCATION (ESTIMATED)</div>
        <div class="flex-row time-allocation-bar">
            <div class="time-alloc-sleep" title="Sleep (33%)"></div>
            <div class="time-alloc-active" title="Work/Activity (50%)"></div>
            <div class="time-alloc-other" title="Other (17%)"></div>
        </div>
        <div class="flex-row gap-sm flex-wrap">
            <div class="flex-row align-center gap-xs"><div class="legend-box time-alloc-sleep"></div> <span class="item-subtitle">SLEEP</span></div>
            <div class="flex-row align-center gap-xs"><div class="legend-box time-alloc-active"></div> <span class="item-subtitle">ACTIVE</span></div>
            <div class="flex-row align-center gap-xs"><div class="legend-box time-alloc-other"></div> <span class="item-subtitle">OTHER</span></div>
        </div>
    `;
    content.appendChild(card);
    elements.resultsSection.appendChild(container);
...
async function renderLivedThroughData() {
    ...
    const deepBox = document.getElementById('timeline-deep');
    if (!histBox) return;
    ...
    try {
        ...
        // 2. Fetch "Deep Dive" events for the birth year
        const yearRes = await fetch(`https://en.wikipedia.org/api/rest_v1/feed/onthisday/all/${m}/${d}`);
        const yearData = await yearRes.json();
        const yearSpecific = yearData.events.filter(e => e.year === year).slice(0, 10);

        if (yearSpecific.length > 0) {
            deepBox.innerHTML = yearSpecific.map((e, i) => `
                <div class="list-item padding-sm-rect font-size-sm border-subtle">
                    <div class="item-num">${i+1}</div>
                    <div class="item-main">
                        <div class="item-title">Happened on Birth Date, ${year}:</div>
                        <a href="https://en.wikipedia.org/wiki/${encodeURIComponent(e.text)}" target="_blank" class="link-no-decor display-block link-dashed">${e.text}</a>
                    </div>
                </div>
            `).join('');
        } else {
            deepBox.innerHTML = '<div class="item-subtitle padding-sm-rect">NO SPECIFIC EVENT ARCHIVED FOR THIS DAY IN YOUR BIRTH YEAR.</div>';
        }
    } catch (e) {
