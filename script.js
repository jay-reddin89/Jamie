...
/**
 * ⚡ Performance: Reduces layout/reflow overhead by skipping redundant DOM updates.
 * Updates textContent only if the value has changed (dirty checking).
 */
...
function startLiveUpdates() {
    // ⚡ Performance: Hoist formatter and clear interval to prevent leaks
    ...
    state.updateInterval = setInterval(() => {
        ...
        updateStat('val-seconds', formatter.format(Math.floor(diff / 1000)));
        updateStat('val-years', age.years.toString());
        updateStat('val-months', formatter.format(Math.floor(diff / 2629800000)));
        updateStat('val-weeks', formatter.format(Math.floor(diff / 604800000)));
        updateStat('val-days', formatter.format(Math.floor(diff / 86400000)));
        updateStat('val-hours', formatter.format(Math.floor(diff / 3600000)));
        updateStat('val-minutes', formatter.format(age.minutes));
...
// ⚡ Performance: Hoisted formatter for reuse across calls
...
function formatLarge(num) {
    ...
    return defaultFormatter.format(Math.floor(num));
}
