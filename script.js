const state = {
    ...
    domCache: {} // Cache for high-frequency DOM lookups
};
...
function loadUserData() {
    ...
    if (savedData) {
        ...
        // Pre-calculate stable date-related values to avoid overhead in high-frequency loops
        if (state.user.dob) {
            state.user.birthDate = new Date(state.user.dob);
            state.user.bornDay = state.user.birthDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
        }
...
function saveUserData() {
    ...
    // Pre-calculate stable date-related values
    state.user.birthDate = new Date(state.user.dob);
    state.user.bornDay = state.user.birthDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
...
function startLiveUpdates() {
     ...
     * 1. Hoist Intl.NumberFormat outside the interval to avoid repeated constructor calls.
     * 2. Implement lazy DOM element caching to eliminate 15+ getElementById calls per second.
     * 3. Implement dirty checking to skip DOM writes when values haven't changed.
    ...
    const updateStat = (id, val) => {
        ...
        if (el && el.textContent != val) el.textContent = val; // Dirty checking
    };

    setInterval(() => {
        if (!state.user.dob || !state.user.birthDate) return;
        const diff = new Date() - state.user.birthDate;
