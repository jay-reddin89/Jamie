...
function startLiveUpdates() {
    ...
    if (bornDayEl && state.user.dobDate) {
        bornDayEl.textContent = state.user.dobDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
    }
    ...
    state.liveUpdateInterval = setInterval(() => {
        ...
        const update = (el, val) => { if (el) el.textContent = val; };

        update(updateEls.seconds, numberFormatter.format(age.seconds));
        update(updateEls.years, age.years);
        update(updateEls.months, numberFormatter.format(Math.floor(diff / 2629800000)));
        update(updateEls.weeks, numberFormatter.format(Math.floor(diff / 604800000)));
        update(updateEls.days, numberFormatter.format(Math.floor(diff / 86400000)));
        update(updateEls.hours, numberFormatter.format(Math.floor(diff / 3600000)));
        update(updateEls.minutes, numberFormatter.format(age.minutes));

        const mins = diff / 60000, days = diff / 86400000;
        update(updateEls.heart, formatLarge(mins * 72));
        update(updateEls.breaths, formatLarge(mins * 14));
        update(updateEls.sleep, formatLarge(days * 8));
        update(updateEls.eat, formatLarge(days * 1.5));
        update(updateEls.blinks, formatLarge(days * 15 * 60 * 16));
    }, 1000);
}
