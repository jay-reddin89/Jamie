...
function loadUserData() {
    ...
    if (savedData) {
        ...
        // Cache parsed date to avoid repeated parsing in live update loop
...
function renderResults() {
    ...
    // Clear performance caches on re-render to avoid stale references
    state.domCache = {};
    state.lastValues = {};
...
function startLiveUpdates() {
    ...
    if (!state.user.dobDate && state.user.dob) state.user.dobDate = new Date(state.user.dob);
    if (!state.user.dobDate) return;

    // One-time calculation for static born day
    const bornDayEl = document.getElementById('val-born-day');
    if (bornDayEl) bornDayEl.textContent = state.user.dobDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
    ...
    state.liveUpdateInterval = setInterval(() => {
        ...
        const update = (id, val) => {
            // Performance: DOM caching + dirty checking to minimize layout/reflow
            let el = state.domCache[id];
            if (!el) el = state.domCache[id] = document.getElementById(id);
        ...
        const f = (n) => state.numberFormatter.format(Math.floor(n));

        update('val-seconds', f(diff / 1000));
        update('val-years', age.years);
        update('val-months', f(diff / 2629800000));
        update('val-weeks', f(diff / 604800000));
        update('val-days', f(diff / 86400000));
        update('val-hours', f(diff / 3600000));
        update('val-minutes', f(age.minutes));
