...
function renderResults() {
    ...
    // Clear any existing live update interval to avoid leaks
 ...
 * Optimized to accept pre-calculated diff to avoid redundant Date allocations.
 */
function calculateAge(dobStr, preCalcDiff = null) {
    const diff = preCalcDiff !== null ? preCalcDiff : (new Date() - new Date(dobStr));
    return {
...
function startLiveUpdates() {
    // Hoist formatters and pre-calculate static values
    ...
    const update = (id, val) => {
        ...
        // Dirty checking: only update DOM if value actually changed
...
// Hoisted formatter for global performance
const largeNumFormatter = new Intl.NumberFormat();

function formatLarge(num) {
    if (num >= 1e9) return (num / 1e9).toFixed(2) + 'B';
    if (num >= 1e6) return (num / 1e6).toFixed(1) + 'M';
    if (num >= 1e3) return (num / 1e3).toFixed(1) + 'k';
    return largeNumFormatter.format(Math.floor(num));
}
