...
function loadUserData() {
    ...
    if (savedData) {
        ...
        // Performance: Pre-calculate date state to avoid parsing in intervals
        if (state.user.dob) {
...
function saveUserData() {
    ...
    // Performance: Update pre-calculated state on save
    state.user.dobDate = new Date(state.user.dob);
    state.user.bornDay = state.user.dobDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();
...
function startLiveUpdates() {
    ...
    setInterval(() => {
        ...
        update('val-minutes', formatter.format(ageMinutes));
        update('val-born-day', state.user.bornDay);

        const days = diff / 86400000;
        update('est-heart', formatLarge(ageMinutes * 72));
        update('est-breaths', formatLarge(ageMinutes * 14));
