...
function updateElement(id, value) {
    if (state.lastValues[id] === value) return;
    ...
    if (state.domCache[id]) {
        state.domCache[id].textContent = value;
...
function startLiveUpdates() {
    ...
    state.liveUpdateInterval = setInterval(() => {
        ...
        updateElement('val-seconds', state.numberFormatter.format(Math.floor(diff / 1000)));
        updateElement('val-years', age.years);
        updateElement('val-months', state.numberFormatter.format(Math.floor(diff / 2629800000)));
        updateElement('val-weeks', state.numberFormatter.format(Math.floor(diff / 604800000)));
        updateElement('val-days', state.numberFormatter.format(Math.floor(diff / 86400000)));
        updateElement('val-hours', state.numberFormatter.format(Math.floor(diff / 3600000)));
        updateElement('val-minutes', state.numberFormatter.format(age.minutes));
        updateElement('val-born-day', state.user.dobDate.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase());

        const mins = diff / 60000, days = diff / 86400000;
        updateElement('est-heart', formatLarge(mins * 72));
        updateElement('est-breaths', formatLarge(mins * 14));
        updateElement('est-sleep', formatLarge(days * 8));
        updateElement('est-eat', formatLarge(days * 1.5));

        updateElement('est-blinks', formatLarge(days * 15 * 60 * 16));
    }, 1000);
}
